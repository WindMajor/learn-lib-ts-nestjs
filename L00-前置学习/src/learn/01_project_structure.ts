...
// 定义装饰器工厂函数
const MyDecoratorFactory = (): ClassDecorator => {
  return (target) => {
    // 装饰逻辑
  };
};
@MyDecoratorFactory() // 装饰器工厂的好处是可以接收参数
class test2 {}

// 模拟 NestJS 的 @Injectable() 装饰器（简化版）。这里的Injectable就是装饰器工厂函数
const Injectable = (): ClassDecorator => {
  return (target: object) => {
    // 在类的原型上存储元数据 —— 「这个类是可注入的」boolean类型，只为true
    ...
    // 这行代码的目的就是：在类型安全的前提下，打印出被装饰的类的名称
  };
};
@Injectable() // 使用示例
class UserService {
...
interface NestCliConfig {
  /** JSON Schema 引用 */
  // $schema 是 JSON 文件的"自述链接"，是 JSON Schema 规范定义的标准字段，所有主流编辑器都支持
...
console.log(obj); // { name: 'NestJS', version: 10 }
...
const bootstrap3 = async (): Promise<void> => {
  // 切换到 Fastify
  const app: NestFastifyApplication = await NestFactory.create<NestFastifyApplication>(AppModule, new FastifyAdapter({ logger: true }));
 ...
 * │   │   │   ├── create-user.dto.ts
 * │   │   │   └── update-user.dto.ts
 * │   │   └── entities/             # 实体（可选，TypeORM需要，Drizzle 使用 schema 定义，不需要单独实体）
 * │   ├── auth/                     # 认证模块
...
// 在当前案例里是什么也没做，最终还是：Omit<OpenAPIObject, 'paths'>
