import { Module, Logger, OnModuleInit, Injectable as NestInjectable } from "@nestjs/common";
import { CatsController } from "./cats.controller";
import { CatsService, DatabaseProvider } from "./cats.service";

// ============================================================
// WHAT: 内存数据库实现——DatabaseProvider 的具体实现
// WHY: 必须放在 @Module 装饰器之前，因为 isolatedModules 要求
//   在类装饰器中引用的类必须先声明
// ============================================================
...
class InMemoryDatabaseService extends DatabaseProvider {
  ...
  query(sql: string): unknown[] {
    ...
    if (sql.includes("cats")) return this.data.get("cats") ?? [];
    return [];
  }
  ...
  generateId(): number { return ++this.idCounter; }
}
